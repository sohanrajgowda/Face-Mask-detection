...
while True:
	...
	# detect faces in the frame and determine their label
	(locs, preds) = detect_and_predict_mask(frame, faceNet, maskNet)

	for (box, pred) in zip(locs, preds):
		(startX, startY, endX, endY) = box
		(mask, withoutMask, withoutProperly) = pred
		...
		label = "{}: {:.2f}%".format(label, max(mask, withoutMask, withoutProperly) * 100)
